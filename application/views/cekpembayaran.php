...
<body>
        ...
        <div class="container text-center">
            ...
            <form class="form-horizontal">
                <div class="form-group">
                    <input name="novirtual" id="novirtual" class="form-control" type="text" placeholder="Masukkan No Virtual">
                </div>
            </form>
        </div>
    </section>
    <section class="blog_area" style="padding-top:25px;padding-bottom:25px;">
        <div class="container">
            <form class="form-horizontal">
                ...
                <label class="control-label col-xs-3">No Judul</label>
                <div class="form-group">
                    <input name="judul" class="form-control" type="text" placeholder="" readonly>
                </div>
                <label class="control-label col-xs-3">Biaya</label>
                <div class="form-group">
                    <input name="biaya" class="form-control" type="text" placeholder="" readonly>
                </div>
                <label class="control-label col-xs-3">Tanggal Pembayaran</label>
                <div class="form-group">
                    <input name="tanggalpembayaran" class="form-control" type="text" placeholder="" readonly>
                </div>
                <!-- <img id="banner" src="" /> -->
            </form>
        </div>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('#novirtual').on('input', function() {

                var novirtual = $(this).val();
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url('home/getpembayaran') ?>",
                    dataType: "JSON",
                    data: {
                        novirtual: novirtual
                    },
                    ...
                    success: function(data) {
                        $.each(data, function(novirtual, nama, judul, biaya, tanggalpembayaran) {
                            $('[name="nama"]').val(data.nama);
                            $('[name="judul"]').val(data.judul);
                            $('[name="biaya"]').val(data.biaya);
                            $('[name="tanggalpembayaran"]').val(data.tanggalpembayaran);
                        });
